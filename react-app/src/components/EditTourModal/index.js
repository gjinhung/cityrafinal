...
function EditTourModal({ tour }) {
    ...
    const [monday, setMon] = useState((tour.dates).includes(1) ? true : false)
    const [tuesday, setTue] = useState((tour.dates).includes(2) ? true : false)
    const [wednesday, setWed] = useState((tour.dates).includes(3) ? true : false)
    const [thursday, setThur] = useState((tour.dates).includes(4) ? true : false)
    const [friday, setFri] = useState((tour.dates).includes(5) ? true : false)
    const [saturday, setSat] = useState((tour.dates).includes(6) ? true : false)
    const [sunday, setSun] = useState((tour.dates).includes(0) ? true : false)
    const [city, setCity] = useState(cities[tour.city_id].city)
    const [history, setHistory] = useState((tour.specialties_id).includes(2) ? true : false)
    const [food, setFood] = useState((tour.specialties_id).includes(1) ? true : false)
    const [adventure, setAdventure] = useState((tour.specialties_id).includes(3) ? true : false)
    const [other, setOther] = useState((tour.specialties_id).includes(4) ? true : false)
